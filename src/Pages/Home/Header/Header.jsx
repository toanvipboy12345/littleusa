...
const Header = () => {
  ...
  const [isCategoriesOpen, setIsCategoriesOpen] = useState(false);
  const [categories, setCategories] = useState([]);
  const [logoUrl, setLogoUrl] = useState(""); // State để lưu URL của logo

  const imageBaseUrl = "http://localhost:8080"; // Base URL để lấy ảnh từ server

  // Lấy logo từ API khi component mount
  useEffect(() => {
    const fetchLogo = async () => {
      try {
        // Gọi API để kiểm tra xem logo.jpeg có tồn tại không
        const response = await axiosInstance.get("/api/uploads/logo.jpeg", {
          responseType: "blob", // Để xử lý file ảnh
        });
        const url = URL.createObjectURL(response.data); // Tạo URL tạm thời từ blob
        setLogoUrl(url);
      } catch (error) {
        console.error("Error fetching logo:", error);
        // Nếu không tìm thấy logo, có thể để logoUrl rỗng hoặc dùng logo mặc định
        setLogoUrl(""); // Hoặc bạn có thể đặt một URL mặc định
      }
    };
        ...
        >
          <Flex justify="space-between" align="center" w="100%">
            ...
            >
              <RouterLink to="/">
                <Image
                  src={logoUrl || `${imageBaseUrl}/api/uploads/logo.jpeg`} // Sử dụng logoUrl nếu có, nếu không thì dùng URL trực tiếp
                  alt="Logo"
                  maxW={{ base: "150px", md: "150px", lg: "180px" }}
                  h="auto"
                  objectFit="contain"
                  fallbackSrc="https://via.placeholder.com/150?text=Logo" // Fallback nếu logo không tồn tại
                />
              </RouterLink>
            </Box>
