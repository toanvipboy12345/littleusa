  ...
  const addToWishlistHandler = async () => {
    ...
    try {
      ...
      const response = await axiosInstance.post("/api/wishlist/add", null, {
        params: {
          userId: userId,
          wishlistToken: userId ? null : wishlistToken,
            ...
            <Box>
              ...
              <HStack spacing={2} wrap="nowrap">
                {variant.sizes.map((size) => {
                  const isOutOfStock = size.quantity === 0; // Kiểm tra số lượng
                  return (
                    <Button
                      key={size.id}
                      onClick={() => !isOutOfStock && handleSizeSelect(size.size)} // Chỉ cho phép chọn nếu còn hàng
                      variant={selectedSize === size.size ? "solid" : "outline"}
                      bg={selectedSize === size.size ? "black" : "transparent"}
                      color={isOutOfStock ? "gray.500" : selectedSize === size.size ? "white" : "black"}
                      borderWidth="1px"
                      p={2}
                      h="40px"
                      borderRadius="0"
                      isDisabled={isOutOfStock} // Vô hiệu hóa nút nếu hết hàng
                      textDecoration={isOutOfStock ? "line-through" : "none"} // Gạch ngang nếu hết hàng
                    >
                      {size.size}
                    </Button>
                  );
